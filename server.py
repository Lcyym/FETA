...
def main():
    ...
    global_model = models_grow.get_model()
    ...
    if common_config.dataset_type == "CIFAR10":
        label_ratio, unlabel_ratio=0.2, 0.8
    # Create model instance
    # train_data_partition, test_data_partition = partition_data(common_config.dataset_type, common_config.data_pattern)
    train_data_partition, test_data_partition = partition_data(common_config.dataset_type, label_ratio, unlabel_ratio, worker_num=worker_num)

    for worker_idx, worker in enumerate(worker_list):
        worker.config.para = init_para
        worker.config.custom["label_data_idxes"] = train_data_partition.use(worker_idx)
        worker.config.custom["unlabel_data_idxes"] = train_data_partition.use(worker_idx+10)
    ...
    # train_loader = datasets.create_dataloaders(train_dataset, batch_size=args.batch_size, shuffle=True, selected_idxs=train_data_partition.use(10))
    ...
    for epoch_idx in range(1, 1+common_config.epoch):
        ...
        # if epoch_idx%100==0:
        #     print("add")
        #     def update_model_construct(old_model,epoch):
        #         new_model = models_grow.get_model(int(epoch/5)).to(device)
        #         # new_model = get_model(int(epoch/5))
        ...
        #     global_model = update_model_construct(global_model,epoch_idx)
        
        global_para = torch.nn.utils.parameters_to_vector(global_model.parameters())
        print("send begin")
        ...
        aggregate_model_para(global_model,worker_list)
        ...
        print("epoch: {}, test loss: {}, test accuracy: {}".format(epoch_idx, test_loss, acc))
...
def iid_partition_10(label_ratio, unlabel_ratio, validation_ratio=0.1):
    worker_num = 10
    partition_sizes = np.ones((10, worker_num*2)) * (1.0) / 20
    # partition_sizes = np.ones((10, worker_num+1))
    for class_idx in range(worker_num):
        for class_idx1 in range(worker_num):
            partition_sizes[class_idx][class_idx1] = 0.02

    for class_idx in range(worker_num):
        for class_idx1 in range(worker_num):
            partition_sizes[class_idx][class_idx1+10] = 0.08

    return partition_sizes

def partition_data(dataset_type, label_ratio, unlabel_ratio, worker_num=10):
    train_dataset, test_dataset = datasets.load_datasets(dataset_type)
    ...
    if dataset_type == "CIFAR100":
        ...
        test_partition_sizes = np.ones((10, worker_num)) * (1 / worker_num)
        if worker_num == 10:
            partition_sizes = iid_partition_10(label_ratio, unlabel_ratio)
        # elif worker_num == 20:
